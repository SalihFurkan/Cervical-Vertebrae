...
# Replica for Augm
# Batch Size = 32, image size = (192,128), 
# total dataset = 1792, total # of classes = 4, 
# training = 1344, validation = 448,
# ep_num = 28, step_tr = 60, step_val = 40, val_acc = 60%
...
tr_shuf 	= (tr_len//batch_size) + 1 #61
tr_rep 		= ((ep_num*step_tr)//tr_shuf) + 1 #48
val_shuf 	= (val_len//batch_size) + 1 #21
val_rep 	= ((ep_num*step_val)//val_shuf) + 1 #92
...
out_rel1_m 	= tf.keras.activations.relu(-out_speical_matched)
out_add 	= tf.keras.layers.Add()([out_rel1, out_rel1_m])
out_norm2 	= tf.keras.layers.BatchNormalization()(out_add)
out_drop1 	= tf.keras.layers.Dropout(0.5)(out_norm2)

out_1 = tf.keras.layers.Concatenate(axis=-1)([out_drop, out_drop1]) 
# From here, ordinary layers
out_2 		= tf.keras.layers.Conv2D(16, (3,3), kernel_regularizer='l2')(out_1)
...
out_conv 	= tf.keras.layers.Conv2D(32, (3,3), kernel_regularizer='l2')(out_7)
out_batch 	= tf.keras.layers.BatchNormalization()(out_conv)
out_rel3 	= tf.keras.activations.relu(out_batch)
...
out_10 = tf.keras.layers.Flatten()(out_9)
...
out_last 	= tf.keras.layers.Dense(6,activation='softmax')(out_13)
...
im_path = 'Replica_for_augm/CS1/12.png'
...
layer_outputs = [layer.output for layer in model.layers[:25]] # Extracts the outputs of the top 6 layers
